...
const createMatrix = () => {
  ...
  const padding = MATRIX_Y_PADDING; // 60px padding
  ...
  for (let row = 0; row < ROWS; row++) {
    for (let col = 0; col < COLUMNS; col++) {
      ...
      symbol.y = -Math.random() * 600 - 200; // Random starting point above

      // Store target position
      symbol.targetY = startY + row * SYMBOL_SIZE;

      // Store start delay for each row (lower rows start first)
      symbol.startDelay = (ROWS - row - 1) * 20; // Lower rows start earlier
